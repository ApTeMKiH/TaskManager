        ...
        <div id="templatemo_main" data-size="${tasks.size()}">
            <c:if test="${tasks.size() == 0}">
                <div class="post_section">
                    <h2>You don't write any tasks.</h2>
                </div>
            </c:if>
            <c:if test="${tasks.size() != 0}">
                <c:forEach items="${tasks}" var="tasks">
                    <div class="post_section">
                        <h2><a href="/tasks/task/${tasks.id}">${tasks.subject}</a></h2>
                            ${tasks.date} <strong>To whom:</strong> ${tasks.userTo.lastName}<span>| Feedbacks: ${tasks.feedbackList.size()}</span>
